import { getCurrentBranch, getCurrentCommitMessage, switchBranch } from "./utils";
...
export async function sync() {
    ...
    const currentStage = tutorialConfig.stages.find(s => s.name === currentBranch);
    if (!currentStage) {
...
export async function pullToThisStage(fromStageBranch: string): Promise<void> {
    const proc = Bun.spawn(['git', 'restore', `--source=${fromStageBranch}`, '--', '.'], {
      stdout: 'pipe',
      stderr: 'pipe',
    });
