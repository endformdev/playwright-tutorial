...
<Aside type="caution">
When you run Playwright in debug mode for the first time, the browser will open and appear frozen on `about:blank`. This is expected! You need to click the "step over" button in the Playwright Inspector window to start stepping through your test.
</Aside>

You can watch your test step by step by using the "step over" button.
<Image src={StepOverDebug} alt="Step over button in Playwright's inspector" width={600} />
...
<Aside type="note">
This step is only necessary if you're running tests against a local Next.js dev application. If you're testing against the remote Vercel deployment, this API endpoint is already available and you can skip this checkout command.
</Aside>
...
<Aside type="tip">
Update the `storageState` path in your existing project configurations. Keep all your other settings (like `testMatch`, `testIgnore`, `dependencies`, and device configurations).
</Aside>
